...
```{r}
topic_model <- lm(pledged_percentage ~ . - degree_of_diff - Pledge_USD - Goal_USD, data = df)

library(broom)

DF = glance(topic_model)[c('r.squared','adj.r.squared', 'nobs')]
is.num <- sapply(DF, is.numeric)
DF[is.num] <- lapply(DF[is.num], round, 3)

stargazer(DF, no.space = TRUE, summary = FALSE)
